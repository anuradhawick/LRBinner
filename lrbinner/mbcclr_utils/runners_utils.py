...
def run_kmers(reads_path, output, k_size, threads):
    """Compute k-mer frequency vectors using pykmertools and write to com_profs."""
    if not os.path.isdir(f"{output}/profiles"):
        os.makedirs(f"{output}/profiles")

    fmt = "fasta" if reads_path.split(".")[-1].lower() in ["fasta", "fna", "fa"] else "fastq"
    computer = kt.OligoComputer(k_size)

    logger.debug(f"Computing {k_size}-mer vectors with pykmertools")

    with open(f"{output}/profiles/com_profs", "w") as out_f:
        for record in SeqIO.parse(reads_path, fmt):
            vec = computer.vectorise_one(str(record.seq))
            out_f.write(" ".join(map(str, vec)) + "\n")
